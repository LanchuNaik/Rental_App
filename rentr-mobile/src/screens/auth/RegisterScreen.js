  ...
  const renderInput = ({ field, label, icon, value, onChangeText, placeholder, secure, toggleSecure, showSecure, keyboardType, autoCapitalize }) => (
    <View style={styles.fieldGroup}>
      ...
      <View style={[styles.inputWrapper, focusedField === field && styles.inputWrapperFocused]}>
        <Ionicons name={icon} size={20} color={focusedField === field ? colors.primary : colors.textMuted} style={styles.inputIcon} />
        <TextInput
          ...
          keyboardType={keyboardType || 'default'} autoCapitalize={autoCapitalize || 'none'} autoCorrect={false}
        />
        {toggleSecure && (
      ...
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          <View style={styles.header}>
            <View style={styles.logoBadge}><Ionicons name="person-add" size={28} color={colors.primary} /></View>
            <Text style={styles.title}>Create account</Text>
            <Text style={styles.subtitle}>Start renting or earning today</Text>
          </View>

          {renderInput({ field: 'name', label: 'Full Name', icon: 'person-outline', value: name, onChangeText: setName, placeholder: 'John Doe', autoCapitalize: 'words' })}
          {renderInput({ field: 'email', label: 'Email', icon: 'mail-outline', value: email, onChangeText: setEmail, placeholder: 'you@example.com', keyboardType: 'email-address' })}
          {renderInput({ field: 'password', label: 'Password', icon: 'lock-closed-outline', value: password, onChangeText: setPassword, placeholder: 'Min. 6 characters', secure: true, showSecure: showPassword, toggleSecure: () => setShowPassword(!showPassword) })}
          ...
          {renderInput({ field: 'confirm', label: 'Confirm Password', icon: 'shield-checkmark-outline', value: confirmPassword, onChangeText: setConfirmPassword, placeholder: 'Re-enter password', secure: true, showSecure: showConfirm, toggleSecure: () => setShowConfirm(!showConfirm) })}
          ...
          <TouchableOpacity style={[styles.primaryButton, loading && styles.buttonDisabled]} onPress={handleRegister} disabled={loading} activeOpacity={0.85}>
            {loading ? <ActivityIndicator color={colors.textInverse} /> : <Text style={styles.primaryButtonText}>Create Account</Text>}
          </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  logoBadge:   { width: 56, height: 56, borderRadius: radius.lg, backgroundColor: colors.primaryLight, alignItems: 'center', justifyContent: 'center', marginBottom: spacing.lg },
  ...
  strengthContainer: { flexDirection: 'row', alignItems: 'center', marginTop: -spacing.sm, marginBottom: spacing.md, gap: spacing.md },
  strengthBar: { flex: 1, flexDirection: 'row', gap: 4 },
  strengthSegment: { flex: 1, height: 4, borderRadius: radius.full },
  strengthLabel: { ...typography.caption, fontWeight: '600', width: 60, textAlign: 'right' },
  checkboxRow: { flexDirection: 'row', alignItems: 'flex-start', marginTop: spacing.sm, marginBottom: spacing.xl, gap: spacing.md },
  checkbox:    { width: 22, height: 22, borderRadius: radius.sm, borderWidth: 2, borderColor: colors.border, alignItems: 'center', justifyContent: 'center', marginTop: 1 },
  checkboxChecked: { backgroundColor: colors.primary, borderColor: colors.primary },
  checkboxText: { flex: 1, ...typography.bodySmall, color: colors.textSecondary, lineHeight: 20 },
